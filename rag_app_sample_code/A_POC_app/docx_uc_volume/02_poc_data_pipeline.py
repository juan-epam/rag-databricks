...
# MAGIC %md
# MAGIC # POC DocX Data Preparation Pipeline
# MAGIC
# MAGIC This is a POC data preperation that provides uses the configuration from `00_config` to build a data pipeline that loads, parses, chunks, and embeds DocX files from a UC Volume into a Databricks Vector Search Index.  
...
# MAGIC %pip install -qqqq -U pypandoc_binary==1.13 databricks-vectorsearch transformers==4.41.1 torch==2.3.0 tiktoken==0.7.0 langchain-text-splitters==0.2.2  mlflow mlflow-skinny
...
# MAGIC %md
# MAGIC ## Step 2: Parse the DocX files into text
# MAGIC
# MAGIC In Bronze/Silver/Gold terminology, this is your Silver table.
# MAGIC
# MAGIC Although not reccomended for your POC, if you want to change the parsing library or adjust it's settings, modify the contents of the `parse_bytes_pypandocdocx` UDF.
