...
runs = mlflow.search_runs(experiment_names=[MLFLOW_EXPERIMENT_NAME], filter_string=f"run_name = '{POC_CHAIN_RUN_NAME}'", order_by=["start_time DESC"], max_results=1, output_format="list")

if len(runs) == 0:
    raise ValueError(f"No runs found with name {POC_CHAIN_RUN_NAME}. Ensure the run name is accurate and try again.")
